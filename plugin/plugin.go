// Package plugin registers distance as a golangci-lint Module Plugin.
//
// Blank-import this package from a custom golangci-lint binary (via
// .custom-gcl.yml) to enable the distance linter. Configuration lives under
// linters.settings.custom.distance in .golangci.yml.
package plugin
...
// Plugin adapts the distance analyzer to golangci-lint's LinterPlugin
// contract.
type Plugin struct {
	loadMode
