...
func Text(report distance.Report, opts TextOptions) string {
	...
	widths := make([]int, 1+len(structuralPackageColumns())+len(table.pkgCols)+len(table.typeCols))
...
func (t *textTable) nodeRow(node *treeNode, summary *treeSummary, label string) {
	row := make([]tableCell, 0, 1+len(structuralPackageColumns())+len(t.pkgCols)+len(t.typeCols))
...
func nodeTypeAggCells(summary *treeSummary, typeCols []string) []tableCell {
	if summary.typesTotal == 0 || len(typeCols) == 0 {
		return nil
	}
...
func (t *textTable) typeRow(
	...
	row = append(row, typeMetricCells(nil, t.typeCols, nil)...)
...
// report, in the fixed metric order.
func reportColumns(distance.Report) []string {
	return nil
}
