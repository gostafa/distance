...
// relative to the other packages in the same subtree. Metrics with no entry
// have no inherent good/bad direction.
...
// a short, readable abbreviation; metric initialisms fall through to their
// uppercase name.
var columnAbbrev = map[string]string{
	metrics.MetricAbstractness: "A",
	metrics.MetricInstability:  "I",
...
func aggregateTree(n *treeNode, summaries map[*treeNode]*treeSummary) *treeSummary {
	...
	if n.pkg != nil {
		summary.pkgsTotal = 1
		collectPackageStats(n.pkg, summary.pkgAgg)
	}
...
// them. Every group mean uses applicable values only. With Explain, the
...
func Text(report distance.Report, opts TextOptions) string {
	...
	header := make([]tableCell, 0, 1+len(table.pkgCols))

	header = append(header, tableCell{text: "PATH", style: ansiDim})
	for _, name := range table.pkgCols {
	...
	widths := make([]int, 1+len(table.pkgCols))
...
func (t *textTable) emitNode(
	...
	for i, child := range node.children {
		...
		t.emitNode(child, summaries, childPrefix, branchGlyph(i, total))
	}
}
...
func (t *textTable) nodeRow(node *treeNode, summary *treeSummary, label string) {
	row := make([]tableCell, 0, 1+len(t.pkgCols))
...
// columnStats aggregates one metric column for subtree means.
...
// from table cells.
func writeNotes(b *strings.Builder, report distance.Report, color bool) {
	wrote := false
...
// boundedColor colors package-row values; only metrics with fixed 0–1
// thresholds get a color.
