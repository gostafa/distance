...
type reportAnalyzer interface {
	...
	Analyze(context.Context, distance.Config) (distance.Report, error)
}

// analyzeFunc adapts a function to reportAnalyzer.
type analyzeFunc func(context.Context, distance.Config) (distance.Report, error)
...
func computeViolations(
	...
	if err != nil {
		return nil, fmt.Errorf("distance policy: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("distance analyze: %w", err)
	}
